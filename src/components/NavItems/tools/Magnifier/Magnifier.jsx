...
const Magnifier = () => {
    ...
    const submitUrl = (src) => {
        trackEvent('submission', 'magnifier', 'image magnifier caa analysis', src, client_id)
        let img = new Image();
        ...
        img.onload = () => {
            ...
            // Get raw image data
            dispatch(setMagnifierResult({url: src, result:canvas.toDataURL('image/png'), notification:false, loading:false}));
            canvas.remove();
        };
        img.onerror = (error) => {
    ...
    return (
        <div>
            <HeaderTool name={keywordAllTools("navbar_magnifier")} description={keywordAllTools("navbar_magnifier_description")} icon={<MagnifierIcon style={{ fill: "#51A5B2" }} width="40px"
            height="40px"/>} />
