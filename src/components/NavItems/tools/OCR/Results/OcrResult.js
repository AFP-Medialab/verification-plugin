...
import { CardContent, TextField, Collapse } from "@material-ui/core";
...
import { OCREdit } from "../OCREdit/OCREdit";
...
const OcrResult = () => {
  ...
  /** Beginning OCREdit Suggestions */
  //Handle OCREdit Expandable Panel Click
  const [expandedPanels, setExpandedPanels] = React.useState([]);
  const handleOCREditPanelClick = (panelIdx) => {
    if (expandedPanels.includes(panelIdx)) {
      setExpandedPanels(expandedPanels.filter((idx) => idx !== panelIdx));
    } else {
      setExpandedPanels([...expandedPanels, panelIdx]);
    }
  };

  /** End OCREdit Suggestions */
  return (
                ...
                >
                  <Grid item xs={6}>
                    <Button
                      ...
                      fullWidth
                      onClick={() => {
                        imageIsUrl ? reverseSearch("yandex") : YandexClick();
                      }}
                    >
                  ...
                  <Grid item xs={6}>
                    <Button
                      ...
                      fullWidth
                      onClick={() => {
                        imageIsUrl ? reverseSearch("bing") : BingClick();
                      }}
                    >
            ...
            <CardContent>
              {result.bounding_boxes.map((ocrResult, index) => (
                <Grid container spacing={2} key={index}>
                  {index > 0 ? (
                    <Grid item xs={12}>
                     <Divider/><Chip variant="outlined" color="primary" size="small" label={index+1} /> 
                      <Box m={2} />
                    </Grid>
                  ) : (
                    <Grid item xs={12}>
                      <Chip variant="outlined" color="primary" size="small" label={index+1} /><Box m={1} />
                    </Grid>
                  )}
                  ...
                  <Grid item xs={12} className={classes.displayFlex}>
                    ...
                    <Grid item xs={6} className={classes.ocrActionAreaRight}>
                      ...
                      <IconButton  onClick={() => handleOCREditPanelClick(index)}>
                  ...
                  <Grid item xs={12}>
                    ...
                    >
                      <Card>
                        ...
                        ></CardHeader>
                        <CardContent>
                          <OCREdit ocrResult={ocrResult} image_id={result.image_id} />
                        </CardContent>
                      </Card>
                    </Collapse>
